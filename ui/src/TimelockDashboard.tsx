...
const isArchiveRpcError = (error: any): boolean => {
  ...
  return msg.includes("missing trie node") || msg.includes("state") || msg.includes("Missing or invalid parameters");
};
...
export default function TimelockDashboard() {
  ...
  // Batch builder state
  const [batchBuilderMode, setBatchBuilderMode] = useState<BatchBuilderMode>("raw");
  ...
  const [batchBuilderArgsText, setBatchBuilderArgsText] = useState<string>(DEFAULT_BATCH_BUILDER_ARGS);
  const [batchBuilderError, setBatchBuilderError] = useState<string>("");
  const [batchBuilderRow, setBatchBuilderRow] = useState<string>(DEFAULT_BATCH_BUILDER_ROW);

  // Batch schedule state
  const [batchRows, setBatchRows] = useState<BatchRow[]>([{ target: "", valueEth: "0", data: "0x" }]);
  ...
  // Configuration state
  const [maxBlocksPerQuery, setMaxBlocksPerQuery] = useState<string>(DEFAULT_MAX_BLOCKS_PER_QUERY);
  ...
  const loadAccessControl = async () => {
    ...
    try {
      ...
      const maxSpan = BigInt(Number(maxBlocksPerQuery || DEFAULT_MAX_BLOCKS_PER_QUERY));
  ...
  const cancelAllLoads = useMemo(
    ...
    [delaysTokenRef, opsTokenRef, aclTokenRef, setLoadingDelays, setLoadingOps, setRolesLoading],
  );
  ...
  const detectDeploymentBlock = async () => {
    ...
    try {
      ...
      if ((e as any)?.message !== "archive-rpc-required") handleError(e, "Detect deployment block");
    } finally {
  ...
  const explorerAddressUrl = useMemo(() => {
    ...
    const chain = wagmiAdapter?.wagmiConfig?.chains?.find((c) => c.id === effectiveChainId);
  ...
  const onExecute = async (entry: OperationEntry) => {
    if (!walletClient || !effectivePublicClient || !account || !contractAddress) return;
  ...
  const buildBatchCalldataFromAbi = () => {
    ...
    try {
      ...
      const idx = Math.max(0, Math.min(Number(batchBuilderRow) || 0, batchRows.length - 1));
  ...
  const computeBatchMinDelay = async () => {
    ...
    try {
        ...
        batchRows.map(async (r) => {
          try {
            const m = await effectivePublicClient!.readContract({
              ...
              abi: networkAbi,
              functionName: "getMinDelay",
              args: [getAddress(r.target) as `0x${string}`, (r.data || "0x") as Hex],
            });
            return m as bigint;
          } catch {
            return 0n;
      ...
      if ((batchDelay || DEFAULT_BATCH_DELAY) === DEFAULT_BATCH_DELAY) setBatchDelay(String(max));
    } catch (e) {
  ...
  const batchRowsStringified = useMemo(() => JSON.stringify(batchRows), [batchRows]);
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto p-4 space-y-6">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">View delays and role holders, schedule actions, execute when ready</p>
          </div>
        ...
        <section className="bg-white rounded border p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={detectingFromBlock || !contractAddress || !effectivePublicClient}
              ...
              <div className="text-xs text-gray-500 mt-1">
                Use a recent block to limit queries. Detect finds the deployment block.
                {explorerAddressUrl ? (
                  <>
                    {" "}
                    Or get it from{" "}
            ...
            <div>
              <label className="block text-xs text-gray-600 mb-1">Max Blocks per Query</label>
              <input
                className="w-full border rounded px-2 py-1"
                value={maxBlocksPerQuery}
                onChange={(e) => setMaxBlocksPerQuery(e.target.value)}
              />
              <div className="text-xs text-gray-500 mt-1">Lower if your RPC has strict limits</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="text-gray-700">
              Global Min Delay:{" "}
              <span className="font-mono">{globalMinDelay !== null ? `${globalMinDelay} s` : "—"}</span>
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <button
              onClick={loadAll}
              disabled={isAnyLoading || !contractAddress || !effectivePublicClient}
              className="px-3 py-1.5 rounded bg-indigo-600 text-white text-sm disabled:opacity-60"
            >
              {isAnyLoading ? "Loading…" : "Load"}
            </button>
            {isAnyLoading && (
              <button onClick={cancelAllLoads} className="px-3 py-1.5 rounded bg-gray-100 border text-sm">
                Cancel
              </button>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-2 text-sm ${
                activeTab === "main" ? "border-b-2 border-indigo-600 text-indigo-600" : "text-gray-600"
              }`}
            >
              Actions
            </button>
            <button
              ...
              className={`px-3 py-2 text-sm ${
                activeTab === "acl" ? "border-b-2 border-indigo-600 text-indigo-600" : "text-gray-600"
              }`}
            >
              Access Control
            </button>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {delayEntries.length === 0 ? (
                    ...
                    <>
                      {delayEntries.map((d) => {
                        const displayTarget = d.target === ZERO_ADDRESS ? "any" : d.target;
                        const displaySelector = d.selector;
                        return (
                          <tr key={d.key} className="border-t">
                            <td className="py-2 pr-4 font-mono text-xs">{displayTarget}</td>
                            <td className="py-2 pr-4 font-mono text-xs">{displaySelector}</td>
                  ...
                  <tr className="border-t bg-gray-50">
                    ...
                    <td className="py-2 pr-4">{globalMinDelay !== null ? globalMinDelay.toString() : "—"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                  ...
                  <div key={i} className="grid grid-cols-12 gap-2 items-end">
                    ...
                    <div className="col-span-1 flex gap-2">
                      <button
                        ...
                        onClick={() => setBatchRows((rows) => rows.filter((_, j) => j !== i))}
                      >
                        −
                      </button>
                    </div>
                ...
                <button
                  ...
                  onClick={() => setBatchRows((r) => [...r, { target: "", valueEth: "0", data: "0x" }])}
                >
                  + Add Row
                </button>
                <details className="mt-2">
                  ...
                  <div className="mt-2 space-y-2 border rounded p-2 bg-gray-50">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">Row index</label>
                        <input
                          className="w-full border rounded px-2 py-1 text-sm"
                          type="number"
                          min={0}
                        ...
                        />
                        <div className="text-xs text-gray-500 mt-1">0 to {Math.max(0, batchRows.length - 1)}</div>
                      </div>
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setBatchBuilderMode(e.target.value as any)}
                        >
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) => setBatchBuilderAbiText(e.target.value)}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setBatchBuilderFnName(e.target.value)}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setBatchBuilderArgsText(e.target.value)}
                          />
                        </div>
                        {batchBuilderError && <div className="text-xs text-red-600">{batchBuilderError}</div>}
                        <button className="px-3 py-1.5 border rounded bg-white" onClick={buildBatchCalldataFromAbi}>
                          Apply to Row
                        </button>
                      </div>
                    )}
                    ...
                    {batchBuilderMode === "raw" && (
                      <div className="text-xs text-gray-600">
                        Edit the row's hex directly in the table above. Builder is optional.
                      </div>
                    )}
                  </div>
                </details>
              ...
              <div className="grid grid-cols-2 gap-3 items-end">
                <div>
                  ...
                  />
                  <div className="text-xs text-gray-500 mt-1">
                    Required min: {batchMinDelay !== null ? `${batchMinDelay} s` : "—"}
                  </div>
                </div>
                            ...
                            {op.txs.map((t) => (
                              <div key={`${op.id}-${t.index.toString()}`} className="p-2 bg-gray-50 rounded border">
                                ...
                                <div className="text-xs">
                                  Target: <span className="font-mono">{t.target}</span>
                                </div>
                                <div className="text-xs">Value: {formatEther(t.value)} ETH</div>
                                <div className="text-xs">
                                  Selector: <span className="font-mono">{selectorFromData(t.data)}</span>
                                </div>
                                <details>
                        ...
                        <td className="py-2 pr-4">
                          {r.members.length === 0 ? (
                            ...
                            <div className="space-y-1">
                              {r.members.map((m) => (
                                <div key={`${r.role}-${m}`} className="font-mono text-xs">
                                  {m}
                                </div>
                              ))}
                            </div>
